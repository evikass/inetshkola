...
## ⚠️ ВАЖНЫЕ ПРАВИЛА ДЕПЛОЯ

### 🔒 ПРАВИЛО GH-PAGES (КРИТИЧЕСКИ ВАЖНО!)

**ДАТА УСТАНОВКИ: 2026-03-06**

**ЗАПРЕЩАЕТСЯ перезаписывать ветку gh-pages если:**
1. Текущая версия gh-pages старше или равна версии master
2. Файлы в gh-pages были созданы корректным способом (см. ниже)

**Правильный алгоритм деплоя на gh-pages:**
```bash
# 1. Создать новую orphan ветку (без истории)
git checkout --orphan gh-pages-new

# 2. Удалить всё из индекса
git rm -rf .

# 3. Получить out папку из master
git checkout master -- out/

# 4. Скопировать содержимое out в корень
cp -r out/* .

# 5. Удалить папку out и другие лишние файлы
rm -rf out public src node_modules .next .env .gitignore

# 6. Добавить favicon из public
git checkout master -- public/favicon.ico public/favicon.png
cp public/favicon.* .
...
# 8. Принудительно отправить как gh-pages
git push -f origin gh-pages-new:gh-pages

# 9. Вернуться на master
git checkout master
```

**ВАЖНО:**
- ❌ НЕ использовать `git merge master` в gh-pages - это вызывает конфликты и добавляет лишние файлы (node_modules и др.)
- ❌ НЕ пушить напрямую master:gh-pages если в истории есть большие файлы (>100MB)
- ✅ Всегда использовать orphan ветку для чистого деплоя
- ✅ Проверить что .nojekyll файл присутствует в корне gh-pages

**Версионирование:**
- При каждом стабильном деплое создавать тег: `git tag -a v1.0.X -m "описание"`
- Теги хранятся в удалённом репозитории и не теряются
