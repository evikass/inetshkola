...
const subjectImages: Record<string, string> = {
  ...
  'safety': '/inetshkola/images/subjects/obzh/hero.svg',
  'biology': '/inetshkola/images/subjects/biology/hero.svg',
  'bio': '/inetshkola/images/subjects/biology/hero.svg',
  ...
  'tech': '/inetshkola/images/subjects/technology/hero.svg',
  ...
  'career': '/inetshkola/images/subjects/career/hero.svg',
  'local': '/inetshkola/images/subjects/local/hero.svg',
  'psychology': '/inetshkola/images/subjects/psychology/hero.svg',
}
    ...
    >
      <Card
        ...
        className={`
          ...
          rounded-2xl sm:rounded-3xl p-3 sm:p-5 md:p-6 border-0 shadow-xl
      ...
      >
        ...
        {image ? (
          <motion.div 
            className="mb-2 sm:mb-3 relative z-10 w-16 h-12 sm:w-20 sm:h-16 md:w-24 md:h-20"
            animate={{ y: [-2, 0, -2] }}
          ...
          <motion.div 
            className="text-3xl sm:text-5xl md:text-6xl mb-2 sm:mb-3 relative z-10"
        ...
        {/* Название предмета */}
        <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-white mb-0.5 sm:mb-1 relative z-10">
          {subject.title}
        </h3>

        {/* Прогресс */}
        <div className="mt-2 sm:mt-3 relative z-10">
          <div className="flex items-center justify-between text-[10px] sm:text-sm text-white/80 mb-0.5 sm:mb-1">
            <span>{completedCount} из {allTopics.length}</span>
            <span>{Math.round(progress)}%</span>
          </div>
          <div className="bg-white/30 rounded-full h-1.5 sm:h-2.5 overflow-hidden">
            <motion.div
        ...
        {/* Звёзды прогресса */}
        <div className="flex gap-0.5 sm:gap-1 mt-2 sm:mt-3 relative z-10">
          {[...Array(5)].map((_, i) => (
            <Star
              key={i}
              className={`w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 transition-all ${
        ...
        {subject.quiz && subject.quiz.length > 0 && (
          <motion.button
            ...
            className="absolute top-2 sm:top-3 right-2 sm:right-3 bg-white/20 hover:bg-white/30 
              text-white rounded-lg sm:rounded-xl px-2 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-sm font-bold
              backdrop-blur-sm transition-all z-10 touch-manipulation"
        ...
        {/* Стрелочка */}
        <div className="absolute bottom-2 sm:bottom-3 right-2 sm:right-3 text-white/60 z-10">
    ...
    return (
      <div className="space-y-3 sm:space-y-4">
        ...
        >
          ...
          <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white">Выбери предмет!</h2>
          <motion.span
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      >
        ...
        <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white">Предметы</h2>
      </motion.div>
